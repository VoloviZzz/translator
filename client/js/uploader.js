function JSUploader() {
    ...
    this.addFiles1 = function(files) {
        $.each(files, function(i, file) {
            ...
            temp.element = baseClass.attachFileToView(temp);
            baseClass.allFiles1.unshift(temp);
        });
    };
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (response.status == 'ok') {;
                        ...
                        // console.log(tmp);
                        // console.log(tmp[3]);
                        $('#uploadFiles_BTN').after('<img id="title_photo" src="/uploads/'+tmp[3]+'">');
                        $('#uploadFiles_BTN').hide();
                    }
                    else {
    ...
    this.uploadFile1 =  function(index) {
        var file = baseClass.allFiles1[index];
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (response.status == 'ok') {;
                        console.log(response.text.path);
                        var tmp = response.text.path.split('/');
                        $('#news_editor').append('<img class="news" src="/uploads/'+tmp[3]+'">');
                        // $('#news_editor').append('<div class="btn btn-primary btn-block" onclick="$(\'#uploadFiles1\').click()">Загрузить фото</div>');
                        $('#uploadFiles_BTN1').remove();
                    }
                    else {
                        console.log(response.errors);
                    }
    ...
    this.uploadAllFiles1 =  function() {
        $.each(baseClass.allFiles, function(i, file) {
            baseClass.uploadFile1(i);
        });
    };
